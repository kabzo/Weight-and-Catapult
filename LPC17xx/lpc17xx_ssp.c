...
int32_t SSP_Write(LPC_SSP_TypeDef *SSPx, SSP_DATA_SETUP_Type *dataCfg)
{
...
int32_t SSP_Read(LPC_SSP_TypeDef *SSPx, SSP_DATA_SETUP_Type *dataCfg)
{
	uint8_t *rdata8 = 0;
	uint16_t *rdata16 = 0;
	uint32_t stat = 0;
...
//SSP_SendData(SSPx, 0xFF);
	while (dataCfg->rx_cnt < dataCfg->length)
	{
		SSP_SendData(SSPx, 0xFF);
		// Check overrun error
